    ...
    public abstract class ElementGroup : Element
    {
        ...
        protected void SetElements(IEnumerable<Element> elements)
        {
            ...
            foreach (var e in elements.Where(e => e != null))
            {
                ...
                {
                    Debug.LogWarning($"WindowElement(FirstLabel[{window.FirstLabel()?.Value}]) cannot be a child of another Element. Please using UI.WindowLauncher() or RajceUI.Build() directly.");
                    continue;
                }
