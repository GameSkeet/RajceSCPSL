...
namespace RajceUI.Elements
{
    public class ListViewItemContainerElement : ElementGroup
    {
        ...
        {
            if (removeState) 
                _itemIndexToElementState.Remove(index);
            if (!_itemIndexToElement.TryGetValue(index, out var element)) return;

            _itemIndexToElement.Remove(index);
        ...
        {
            foreach (KeyValuePair<int, Element> kvp in _itemIndexToElement)
            {
                if (_itemIndexToElementState.ContainsKey(kvp.Key)) continue;

                _itemIndexToElementState[kvp.Key] = ElementState.Create(kvp.Value);
            }
        }
        ...
        {
            if (!_itemIndexToElementState.TryGetValue(fromIndex, out var state))
            {
                state = CreateElementState(fromIndex);
            }
            else _itemIndexToElementState.Remove(fromIndex);
