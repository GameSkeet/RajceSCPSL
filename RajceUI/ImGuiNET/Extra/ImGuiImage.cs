...
namespace RajceUI.ImGuiNET
{
    ...
    public static partial class ImGuiUn
    {
        ...
        {
            ImGui.ImageButton(Guid.NewGuid().ToString(), (IntPtr)GetTextureId(tex), new Vector2(tex.width, tex.height));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ImageButton(Texture tex, Vector2 size)
        {
            ImGui.ImageButton(Guid.NewGuid().ToString(), (IntPtr)GetTextureId(tex), size);
        }
        ...
        {
            SpriteInfo info = GetSpriteInfo(sprite);
            ImGui.ImageButton(Guid.NewGuid().ToString(), (IntPtr)GetTextureId(info.texture), info.size, info.uv0, info.uv1);
        }
        ...
        {
            SpriteInfo info = GetSpriteInfo(sprite);
            ImGui.ImageButton(Guid.NewGuid().ToString(), (IntPtr)GetTextureId(info.texture), size, info.uv0, info.uv1);
        }
    }
}
